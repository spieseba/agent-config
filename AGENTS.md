...
**Tradeoff:** The following guidelines bias toward caution over speed. For trivial tasks (typos, formatting, mechanical one-line fixes), use judgment.
...
Before implementing:
- State your assumptions explicitly. If anything is unclear, stop and ask — name what's confusing rather than guessing.
...
- Improve adjacent code only as a separate, flagged change — never folded silently into the requested diff.
...
## Communication
- When reporting to the user, be extremely concise; sacrifice grammar for concision. Concision compresses phrasing, never coverage: still list every assumption, tradeoff, and open question, a terse line each.
